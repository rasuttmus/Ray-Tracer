...
{
    glm::dvec3 p0(0.0 * size + position.x, 0.99, -1.0 * size + position.z);
    glm::dvec3 p1(1.0 * size + position.x, 0.99, -1.0 * size + position.z);
    glm::dvec3 p2(1.0 * size + position.x, 0.99, 0.0 * size + position.z);
    glm::dvec3 p3(0.0 * size + position.x, 0.99, 0.0 * size + position.z);
...
glm::dvec3 Light::randomPosition() {
   ...
   /* randPos.x = static_cast<double>(rand() % (lightObject->corners.at(1).x - lightObject->corners.at(0).x) + lightObject->corners.at(0).x);
    randPos.y = 0.9999;
    randPos.z = static_cast<double>(rand() % (lightObject->corners.at(1).z - lightObject->corners.at(2).z) + lightObject->corners.at(2).z);
    std::cout << "rand: (" << randPos.x << ", " << randPos.y << ", " << randPos.z << ")" << std::endl << std::endl;*/
    //return glm::dvec3(0.5, 0.99, -0.5);
    return randPos;
}
